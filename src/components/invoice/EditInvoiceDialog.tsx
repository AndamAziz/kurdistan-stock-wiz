...
export function EditInvoiceDialog({ open, onOpenChange, invoice, onSuccess }: EditInvoiceDialogProps) {
  ...
  const handleConfirmDeleteItem = () => {
    if (deletingItemIndex !== null) {
      setItems(prev => {
        const updated = [...prev];
        updated[deletingItemIndex] = { ...updated[deletingItemIndex], isDeleted: true };
        return updated;
      });
    }
  ...
  const deletingItem = deletingItemIndex !== null ? items[deletingItemIndex] : null;
  ...
  return (
    <>
      ...
      <AlertDialog open={deletingItemIndex !== null} onOpenChange={(open) => !open && setDeletingItemIndex(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>دڵنیای لە سڕینەوە؟</AlertDialogTitle>
            <AlertDialogDescription>
              ئایا دڵنیای دەتەوێت مادەی "{deletingItem?.item_name}" لە ئینڤۆیس بسڕیتەوە؟
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
